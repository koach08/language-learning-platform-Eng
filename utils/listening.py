...
A: You're so helpful. I really appreciate it.""",
        "questions": [
            {"question": "Where did Yuki transfer from?", "options": ["Tokyo", "Osaka", "Kyoto", "Nagoya"], "correct": "Osaka"},
            {"question": "What does Sarah offer to show Yuki?", "options": ["The classroom", "The campus", "The city", "Her notes"], "correct": "The campus"},
            {"question": "Where is the bookstore located?", "options": ["Next to the library", "Next to the cafeteria", "Near the station", "In the main building"], "correct": "Next to the cafeteria"}
        ]
    },
    "news_report": {
...
That's all for today's tech news. Stay tuned for weather and sports after the break.""",
        "questions": [
            {"question": "What features does the new smartphone have?", "options": ["Larger screen and better speakers", "Improved camera and longer battery", "Faster processor and more storage", "Better display and wireless charging"], "correct": "Improved camera and longer battery"},
            {"question": "What percentage of employees work from home at least part-time?", "options": ["20 percent", "30 percent", "40 percent", "50 percent"], "correct": "40 percent"},
            {"question": "How many users did the new social media app gain?", "options": ["1 million", "5 million", "10 million", "20 million"], "correct": "10 million"}
        ]
    },
    "lecture_excerpt": {
...
For next class, I'd like you to read chapter three, which discusses the biological basis of memory. We'll explore how memories are actually formed in the brain.""",
        "questions": [
            {"question": "How long does sensory memory typically last?", "options": ["Less than a second", "About 10 seconds", "About 30 seconds", "Several minutes"], "correct": "Less than a second"},
            {"question": "What is the typical capacity of short-term memory?", "options": ["Three items", "Five items", "Seven items", "Ten items"], "correct": "Seven items"},
            {"question": "What is the process called when information moves to long-term memory?", "options": ["Encoding", "Consolidation", "Retrieval", "Recognition"], "correct": "Consolidation"}
        ]
    },
    "job_interview": {
...
B: Yes, I'd like to know more about the team I would be working with.""",
        "questions": [
            {"question": "What did the candidate study at university?", "options": ["Marketing", "Business administration", "Computer science", "Communications"], "correct": "Business administration"},
            {"question": "What was the result of the candidate's social media campaign?", "options": ["10% increase in followers", "20% increase in followers", "30% increase in followers", "50% increase in followers"], "correct": "30% increase in followers"},
            {"question": "What does the candidate hope to do in five years?", "options": ["Start their own company", "Lead a marketing team", "Move to another country", "Get an MBA"], "correct": "Lead a marketing team"}
        ]
    }
}
...
For dialogues, use format "A: text" and "B: text" to indicate different speakers."""
    ...
    user_prompt = f"""Create a listening exercise based on this request: "{prompt}"
...
{{
    ...
    "category": "<Category>",
    "level": "{level}",
    "is_dialogue": <true if conversation, false for narration>,
    "speakers": {{"A": "<Speaker A>", "B": "<Speaker B>"}} or null,
    "script": "<Full script>",
...
def generate_quiz_from_script(script, level="B1"):
    """スクリプトから理解度クイズを生成"""
    client = get_openai_client()
    prompt = f"""Create 3 comprehension questions for this listening script. Level: {level}
...
{{
    "questions": [
        {{
            ...
            "options": ["<A>", "<B>", "<C>", "<D>"],
            "correct": "<correct answer that matches exactly one of the options>",
...
Rules:
- Questions MUST require actually listening to the script to answer (not general knowledge)
- Use specific details, names, numbers, or facts from the script
- Make wrong answers plausible but clearly incorrect based on the script
- The "correct" value must exactly match one of the four options
"""
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an English quiz creator. Create questions that genuinely test comprehension of the given text. Respond in valid JSON."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.5,
            response_format={"type": "json_object"}
...
{{
    ...
    "key_vocabulary": [
        {{
            "word": "<important word/phrase>",
            "meaning": "<日本語の意味>",
            "example_from_video": "<how it was used>"
        }}
    ],
...
"""
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an English learning material creator for Japanese learners. Respond in valid JSON."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
            response_format={"type": "json_object"}
...
{{
    ...
    "recommendations": "<Tips for studying with this video>"
}}
"""
    try:
        response = client.chat.completions.create(
...
def get_voice_for_speaker(speaker, speaker_info=None):
    ...
    if speaker_info:
        ...
        if "female" in info_lower or "woman" in info_lower or "girl" in info_lower:
            return "nova" if speaker in ["A", "C"] else "shimmer"
        elif "male" in info_lower or "man" in info_lower or "boy" in info_lower:
            return "echo" if speaker in ["A", "C"] else "onyx"
    default_mapping = {"A": "nova", "B": "echo", "C": "shimmer", "D": "onyx", "narrator": "alloy"}
